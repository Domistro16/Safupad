...
export function TradingInterface({ token, pancakeSwapPrice, pancakeSwapPriceInBNB, graduatedToPancakeSwap = false }: TradingInterfaceProps) {
  ...
  const effectivePrice = (graduatedToPancakeSwap && pancakeSwapPrice) ? pancakeSwapPrice : token.currentPrice;
  const effectivePriceInBNB = (graduatedToPancakeSwap && pancakeSwapPriceInBNB) ? pancakeSwapPriceInBNB : null;
  ...
  const isBuyDisabled = isProjectRaise ? graduatedToPancakeSwap : token.graduated;
    ...
    try {
      if (tradeType === "buy") {
        ...
        if (isProjectRaise && graduatedToPancakeSwap) {
          ...
          const minBNBOut = (expectedBnb * (1 - slippagePercent / 100)).toFixed(18);
          ...
          const contract = new ethers.Contract(token.contractAddress, ['    function approve(address spender, uint256 value) external returns (bool)'], signer)

          const approve = await contract.approve(sdk.launchpad.address,Number(amount + 1).toString())
          await approve.wait()
          ...
          toast.success(`Successfully sold ${amount} ${token.symbol} on PancakeSwap!`);
  ...
  const calculateBnb = (tokenAmount: string) => {
    ...
    const bnbRate = sdk.priceOracle.usdToBNB(ethers.parseEther(effectivePrice.toString()));
      ...
      {isPancakeSwapPrice && (
        <div className="mb-4 p-3 bg-secondary/10 border-2 border-secondary/30 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-secondary">Using PancakeSwap Price</span>
          </div>
        </div>
      )}
        ...
        <TabsContent value="buy" className="space-y-4">
          ...
          <div className="space-y-2 p-4 bg-muted/50 rounded-lg text-sm">
            <div className="flex justify-between">
              <span className="text-muted-foreground">Price {isPancakeSwapPrice && "(PancakeSwap)"}</span>
              <span className="font-medium">{formatPrice(effectivePrice)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Trading Fee ({token.launchType === "project-raise" ? "2%" : "2%"})</span>
        ...
        <TabsContent value="sell" className="space-y-4">
          ...
          <div className="space-y-2 p-4 bg-muted/50 rounded-lg text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">Trading Fee ({token.launchType === "project-raise" ? "2%" : "2%"})</span>
              <span className="font-medium">
                {bnbAmount ? (parseFloat(bnbAmount) * (token.launchType === "project-raise" ? 0.02 : 0.02)).toFixed(4) : "0.0000"} BNB
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Slippage</span>
        ...
        <div className="grid grid-cols-4 gap-2">
          {["0.1", "0.5", "1", "5"].map((val) => (
            <Button
              ...
              onClick={() => {
                setBnbAmount(val);
                const tokens = calculateTokens(val);
                setAmount(tokens);
              }}
              disabled={isTrading || isBuyDisabled}
            >
