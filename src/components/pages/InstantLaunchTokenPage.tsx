...
export default function InstantLaunchTokenPage() {
  ...
  useEffect(() => {
    ...
    const tfSeries: Record<string, { d: string; px: string; chg: string; seed: number }> = {
    ...
    function lineToArea(d: string){
      const pts = d.replace(/^M/, 'L')
      return `M0,40 ${pts} L100,40 Z`
    }

    function setTF(key: string){
      ...
      if (lineAnim) {
        ...
        try { (lineAnim as SVGAnimateElement).beginElement() } catch {}
      }
      if (areaAnim) {
        areaAnim.setAttribute('values', `${pxArea.getAttribute('d') || lineToArea(fromD)};${lineToArea(toD)}`)
        try { (areaAnim as SVGAnimateElement).beginElement() } catch {}
      }
    ...
    function samplePriceAt(x: number){
      const b = basePrice()
      const seed = (tfSeries[activeTF]?.seed || 1)
    ...
    const onMove = (e: MouseEvent) => {
      ...
      crossV.setAttribute('x1', String(clampedX))
      crossV.setAttribute('x2', String(clampedX))
      ;(crossV as SVGElement).style.opacity = '1'
      ...
      const textWidth = (priceText as SVGTextElement).getComputedTextLength()
      const pad = 1.6
      ...
      priceBg.setAttribute('width', String(boxW))
      priceBg.setAttribute('height', String(boxH))
      priceBg.setAttribute('x', String(px))
      priceBg.setAttribute('y', String(py))

      priceText.setAttribute('x', String(px + pad))
      priceText.setAttribute('y', String(py + 3.05))

      priceGroup.setAttribute('opacity', '1')
    }
    const onLeave = () => {
      if (crossV) (crossV as SVGElement).style.opacity = '0'
    ...
    function setupList({ listId, pagesId, prevId, nextId, data, kind }: { listId: string; pagesId: string; prevId: string; nextId: string; data: any[]; kind: string }) {
      let page = 0
      const size = 20
      const list = scope.querySelector(`#${listId}`)
      const pages = scope.querySelector(`#${pagesId}`)
      const prev = scope.querySelector(`#${prevId}`) as HTMLButtonElement | null
      const next = scope.querySelector(`#${nextId}`) as HTMLButtonElement | null
    ...
    const amountInput = scope.querySelector('#amountInput') as HTMLInputElement | null
    const slider = scope.querySelector('#amountSlider') as HTMLInputElement | null
