...
import { CreatorFeeInfo } from "@safupad/sdk";
...
export default function Home() {
        ...
        const items: Token[] = await Promise.all(
          addresses.map(async (addr: string, idx: number): Promise<Token> => {
            try {
              ...
              let creatorFeeInfo: CreatorFeeInfo | null = null;
              try {
                creatorFeeInfo = await sdk.bondingDex.getCreatorFeeInfo(addr);
                console.log(
                  `Creator fee info for ${tokenInfo.name}:`,
                  creatorFeeInfo
                );
              } catch (err) {
                console.warn(
                  `Could not fetch creator fee info for ${addr}:`,
                  err
                );
              }
              ...
              try {
                ...
                // Get holder count
                holderCount =
                  await sdk.bondingDex.getEstimatedHolderCount(addr);

                // Get 24h price change
                try {
                  const priceChangeData =
                    await sdk.bondingDex.get24hPriceChange(addr);
                  priceChange24h = priceChangeData.priceChangePercent;
                  console.log(
                    `Price change for ${tokenName}: ${priceChange24h}%`
                  );
                } catch (error) {
                  console.warn(
                    `Could not fetch price change data for ${addr}:`,
                    error
                  );
                }
                ...
                console.warn(
                  `Could not fetch volume/holder data for ${addr}:`,
                  error
                );
              }
  ...
  const filteredTokens = useMemo(() => {
    ...
    // Filtering
    if (filterBy === "project")
      list = list.filter((t) => t.launchType === "project-raise");
    if (filterBy === "instant")
      list = list.filter((t) => t.launchType === "instant-launch");
    if (filterBy === "activeRaises")
      list = list.filter(
        (t) => t.launchType === "project-raise" && t.status === "active"
      );
    if (filterBy === "trading")
      list = list.filter((t) => t.graduated === false);
    if (filterBy === "graduated")
      list = list.filter((t) => t.graduated === true);
    ...
    if (sortBy === "newest") {
      ...
      list.sort((a, b) => {
        const ea =
          a.launchType === "project-raise"
            ? (a.projectRaise?.endTime?.getTime?.() ?? Infinity)
            : Infinity;
        const eb =
          b.launchType === "project-raise"
      ...
      <footer className="mt-12 bg-card/30 backdrop-blur-sm relative overflow-hidden">
        <div className="container mx-auto px-4 py-10 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <p className="text-sm text-accent/80 font-semibold tracking-wide !whitespace-pre-line">
              © 2025 Level3 Lab
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-4 md:gap-6 text-sm">
              <a
                href="#"
                className="text-accent/90 hover:text-accent underline-offset-4 hover:underline transition-colors"
              >
                Docs
              </a>
              <a
